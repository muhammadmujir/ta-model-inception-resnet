        ...
        if not load_weights:
            ...
            for i in range(totalLayer):
                if i < breakPoint[0]:
                    ...
                    list(self.frontend2.state_dict().items())[j][1].data[:] = list(mod.state_dict().items())[i][1].data[:]
                    j += 1
                else:
                    list(self.frontend3.state_dict().items())[k][1].data[:] = list(mod.state_dict().items())[i][1].data[:]
                    k += 1
